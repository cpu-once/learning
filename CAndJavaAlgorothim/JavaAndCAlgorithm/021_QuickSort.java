public class QuickSort {
  ...
  public static int divideArray(int[] a, int head, int tail) {
    ...
    right = tail; // 배열 끝 요소부터 앞요소로 훑어 가는 위치

    // 기준값 a[head]보다 적은 요소를 앞쪽으로, 큰 요소를 뒤쪽으로 이동
    while (true) {
      // 배열을 첫 요소 + 1 부터 뒤쪽으로 훑어가,
      // 기준값보다 큰 요소를 찾음
      ...
      // 기준값보다 큰 a[left]와 기준값보다 적은 a[right]를 교환
      temp = a[left];
      a[left] = a[right];
      a[right] = temp;
