...
// 해시 테이블의 실체가 되는 배열(요소 수를 10개로 정함)
...
int main() {
  ...
  do {
    ...
    if (hashTable[pos] == -1) {
      // 해시테이블이 가득 차지 않았다면 데이터를 저장
      hashTable[pos] = data;
      printf("hashTable[%d]에 %d을(를) 저장합니다.\n", pos, data);
    } else {
  ...
  do {
    ...
    if (hashTable[pos] == data) {
      printf("hashTable[%d]값은 %d 이므로, 발견한 위치를 표시합니다.\n", pos, data);
      printf("%d번째에서 발견되었습니다.\n", pos);
    } else {
      printf("hashTable[%d]값은 %d 이므로, '찾을 수 없습니다.' 를 표시하빈다.\n", pos, hashTable[pos]);
      printf("찾을 수 없습니다.\n");
    }
  } while (TRUE);
