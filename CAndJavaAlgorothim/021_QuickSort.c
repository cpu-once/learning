...
int divideArray(int a[], int head, int tail) {
  ...
  /*
   * 1. 기준값이 되는 요소를 하나 선택합니다.
   * 2. 나머지 요소를 기준값보다 작은 값과 큰 값으로 그룹 나누기를 반복합니다.
   * 3. 그룹으로 나누어진 데이터가 하나가 될 때까지 1과 2를 반복합니다.
   *
   * 퀵 정렬은 기준값(pivot)이 되는 요소를 하나 선택해, 나머지 요소들을 기준값보다 작은 값과 큰 값으로 나누기를 반복하여
   * 전체를 정렬합니다.
   *
   * intro sort 또는 java 나 cpp standard lib sort 구현 알고리즘 찾아 볼 것
   */
